...
<body>
    ...
    <% if (rows.length > 0) { %>
      <% for(var i = 0; i < rows.length; i++) { %>
        <tr>
          ...
          <td>￥<%:=rows[i].amount %></td>
          <td>
            <a class="btn-link" href="<?=DJADMIN_URL?>mshop/order/retail_detail/<%:=rows[i].serial_number %>">详情</a>
          </td>
        </tr>
      <% } %>
    <%} else {%>
  ...
  <?=static_original_url('djadmin/mshop/js/retail_order_list.min.js');?>
</body>
</html>
