 ...
 * @Last Modified time: 2018-07-14 11:20:41
 */
...
* wm小程序
*/
class Xcx_meal_config_api extends wm_service_controller
{
    ...
    {  
        ...
        $data['info']=$m_xcx_app;
        //use_status 0代表无需操作,1代表首次注册,2代表需要升级
        ...
        $data['domain']= $this->get_shop_base_url().'/';
    ...
    public function generate()
    {
        ...
        try {
            ...
            $item['title'] = '首页';
            $item['address'] = 'pages/index/index';
            $item_list = [];
    ...
    public function save_secreat()
    {
        ...
        $xcxAppDao = XcxAppDao::i();
        if($xcxAppDao->update(['app_secreat'=>$fdata['app_secreat']],['aid'=>$this->s_user->aid,'app_id'=>$fdata['app_id']]) !== false)
        {
            $this->json_do->out_put();
        }
        else
     ...
     * @return [type] [description]
     */
    public function save_verify_file()
    {
        $rule = [
            ['field' => 'app_id', 'label' => '小程序app_id', 'rules' => 'trim|required'],
            ['field' => 'verify_file_name', 'label' => '验证文件原名称', 'rules' => 'trim|required'],
            ['field' => 'verify_file_path', 'label' => '验证文件路径', 'rules' => 'trim|required'],
        ];
        $this->check->check_ajax_form($rule);
        $fdata = $this->form_data($rule);
        $xcxAppDao = XcxAppDao::i();
        if($xcxAppDao->update(['verify_file_name'=>$fdata['verify_file_name'],  'verify_file_path'=>$fdata['verify_file_path']],['aid'=>$this->s_user->aid,'app_id'=>$fdata['app_id']]) !== false)
        {
            $this->json_do->out_put();
        }
        else
