        ...
        <div class="clearfix mb20">
          <div class="form-inline pull-left">
            <div class="form-group">
              <select id="active_state" class="form-control active_state" name="active_state" onchange="changeCate()">
          ...
          <div class="btn-box pull-right">
            <a href="<?=ADMIN_URL?>wsc_main_article/add" class="btn btn-primary"><span class="iconfont icon-add"></span>新增公告</a>
          </div>
        </div>
        <div id="shopCon">
          <table class="table">
          ...
          <div id="wscPage" class="m-pager"></div>
        </div>
      </div>
    </div>
  </div>
</div>
<div id="delWscModal" class="modal fade" tabindex="-1" role="dialog">
...
<script id="wscTpl" type="text/html">
  <% if(rows.length > 0) { %>
    <% for(var i = 0; i < rows.length; i++) { %>
      <tr>
        ...
        <td>
          <a class="btn-link btn-primary" href="<?=ADMIN_URL?>wsc_main_article/detail/<%:= rows[i].id%>">查看</a>
          <a class="btn-link btn-primary" href="<?=ADMIN_URL?>wsc_main_article/edit/<%:= rows[i].id%>">编辑</a>
          <a class="btn-link btn-danger" href="javascript:;" onclick="delWsc(<%:= rows[i].id%>)">删除</a>
          <% if(rows[i].is_index == '1') { %>
  ...
  <?=static_original_url('admin/vshop/js/wsc_list.js');?>
</body>
</html>