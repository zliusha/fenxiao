      ...
      <div class="main-body-inner">
        <div class="clearfix mb20">
          ...
          <div class="btn-box pull-right">
            <a href="<?=ADMIN_URL?>wm_main_xcx_version/add" class="btn btn-primary"><span class="iconfont icon-add"></span>新增版本</a>
          </div>
        </div>
        <div id="shopCon">
          <table class="table">
  ...
  <% if(rows.length > 0) { %>
    <% for(var i = 0; i < rows.length; i++) { %>
      <tr>
        ...
        <td><%:= rows[i].user_version%></td>
        <td><%:= rows[i].template_id%></td>
          <td><%:= rows[i].user_desc%></td>
        <td>
          <% if(rows[i].status == '1') { %>
          <span>发布</span>
          <% } else { %>
          ...
          <td><%:= rows[i].time%></td>
        <td>
<!--          <a class="btn-link btn-primary" href="--><?//=ADMIN_URL?><!--wm_main_xcx_version/detail/<%:= rows[i].id%>">查看</a>-->
          <a class="btn-link btn-primary" href="<?=ADMIN_URL?>wm_main_xcx_version/edit/<%:= rows[i].id%>">编辑</a>
  ...
  <?=static_original_url('admin/vshop/js/ydb_xcx_version_list.js');?>
</body>
</html>