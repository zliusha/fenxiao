...
class MicroPay
{
	...
	public function pay($microPayInput)
	{
		...
		log_message('error',__METHOD__.'-'.json_encode($result,JSON_UNESCAPED_UNICODE));
		//如果返回成功
		if(!array_key_exists("return_code", $result)
			|| !array_key_exists("result_code", $result))
		{
			log_message('error',__METHOD__.'-接口调用失败！-'.json_encode($result,JSON_UNESCAPED_UNICODE));
	...
	{
		log_message('error',__METHOD__.'-'.$depth);
		if($depth > 10){
			return false;
		}
