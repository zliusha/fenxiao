...
class JsApiPay
{
    ...
    private function __CreateOauthUrlForOpenid($code)
    {
        ...
        $bizString = $this->ToUrlParams($urlObj);
        if (WXPAY_LOGIN_TYPE == 1) {
            // 第三方授权获取token
            $urlObj['component_appid'] = WXPAY_COMPONENT_APPID;
            $urlObj['component_access_token'] = WXPAY_COMPONENT_ACCESS_TOKEN;
            return "https://api.weixin.qq.com/sns/oauth2/component/access_token?" . $bizString;
        } else {
            $urlObj["secret"] = WXPAY_APPSECRET;
