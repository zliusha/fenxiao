...
class Coupon extends mshop_controller
{
    ...
    public function index()
    {
        ...
        $list = $wm_coupon_query_bll->query([
            'status' => isset($fdata['status']) ? $fdata['status'] : '',
    ...
    public function load()
    {
        ...
        $coupon_wxbind = $wmCouponWxbindDao->getOneArray(['open_id' => $this->s_user->openid, 'aid' => $this->aid]);
        ...
        $ret['coupon'] = null;
        foreach ($ret['list'] as $key => $row) {
            if ($this->s_user->openid == $row['open_id']) {
                $ret['coupon'] = $row;
            }
        }
        ...
        if ($ret['mobile']) {
            ...
            if ($wmCouponDao->getCount(['aid' => $this->aid, 'tid' => $fdata['tradeno'], 'open_id' => $this->s_user->openid, 'type' => 1, 'time >=' => $_start_time, 'time <=' => $_end_time]) >= 5) {
        ...
        // 优惠券已领完
        if ($wmCouponDao->getCount(['tid' => $fdata['tradeno'], 'aid' => $this->aid, 'type' => 1, 'open_id !=' => $this->s_user->openid]) >= $ret['setting']['quantity']) {
    ...
    public function _changeMobile($mobile)
    {
        ...
        if ($wmCouponWxbindDao->getCount(['open_id' => $this->s_user->openid, 'aid' => $this->aid]) > 0) {
            // 更新
            $result = $wmCouponWxbindDao->update(['mobile' => $mobile, 'update_time' => time()], ['open_id' => $this->s_user->openid, 'aid' => $this->aid]);
        } else {
            // 创建
    ...
    public function pickup()
    {
        ...
        // 校验当前网址aid和订单号aid是否一致
        $wmOrderDao = WmOrderDao::i($this->aid);
        $order = $wmOrderDao->getOneArray(['tid' => $fdata['tradeno']], 'aid');
        ...
        // 检查是否已经领取
        $wmCouponDao = WmCouponDao::i($this->aid);
        if ($wmCouponDao->getCount(['tid' => $fdata['tradeno'], 'aid' => $this->aid, 'open_id' => $this->s_user->openid, 'type' => 1]) > 0) {
        ...
        $result = $wmCouponDao->create([
            ...
            'wx_nick' => $this->s_user->nickname ? $this->s_user->nickname : '',
            'open_id' => $this->s_user->openid,
    ...
    public function loadFollow()
    {
        ...
        // 获取已领红包记录
        $wmCouponDao = WmCouponDao::i($this->aid);
        $ret['coupon'] = $wmCouponDao->getOneArray(['open_id' => $this->s_user->openid, 'aid' => $this->aid, 'type' => 2], '*', 'id asc');
    ...
    public function pickupFollow()
    {
        ...
        if ($fdata['mobile'] == '13588340300') {
            log_message('error', 'pickupFollow_' . json_encode($this->s_user));
        }
        ...
        // 检查是否已经领取
        $wmCouponDao = WmCouponDao::i($this->aid);
        if ($wmCouponDao->getCount(['aid' => $this->aid, 'open_id' => $this->s_user->openid, 'type' => 2]) > 0) {
            $this->json_do->set_error('004', '已领取过优惠券');
        }
        ...
        for ($i = 0; $i < $coupon_setting['quantity']; $i++) {
            // 优惠券金额计算
            if ($coupon_setting['amount_type'] == 1) {
                $coupon_amount = $coupon_setting['amount'];
            } else if ($coupon_setting['amount_type'] == 2) {
    ...
    public function detail()
    {
        $wmCouponSettingDao = WmCouponSettingDao::i($this->aid);
        $time = time();
        $_list = $wmCouponSettingDao->getAllArray("aid={$this->aid} AND is_open=1 AND status=2 AND start_time > {$time} AND end_time > {$time}");

        $data['lb_info'] = array_find($_list, 'type', 1);//裂变优惠
        $data['gz_info'] = array_find($_list, 'type', 2);//关注领取
