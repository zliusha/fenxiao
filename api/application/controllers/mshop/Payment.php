...
class Payment extends mshop_controller
{
    // SCRM接口请求参数
    public $scrmParams = [];
    ...
    public function weixin()
    {
        ...
        if ($payorder['return_code'] == 'FAIL') {
            log_message('error', __METHOD__.'msg:'.json_encode($payorder));
    ...
    public function weixin_result()
    {
        ...
        $wmPaymentRecordDao = WmPaymentRecordDao::i($this->aid);
        $result = $wmPaymentRecordDao->getOne(['code' => $fdata['tid'], 'aid'=>$this->aid, 'gateway' => 'weixin', 'uid' => $this->s_user->uid], 'status', 'id desc');
    ...
    public function balancepay()
    {
        ...
        if ($fdata['type'] == 'order') {
            ...
            // 支付订单记录
            $record = $wmPaymentRecordDao->getOne(['aid'=>$this->aid,'code' => $order->tid, 'gateway' => 'balance', 'uid' => $this->s_user->uid]);
        ...
        if ($result['code'] != 0) {
             log_message('error', __METHOD__.' params:'.json_encode($this->scrmParams).' result:'. json_encode($result));
