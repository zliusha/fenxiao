...
class Address extends xcx_user_controller
{
   ...
   */
    public function info()
    {
        ...
        $receiver_address_id = $this->input->post_get('receiver_address_id');
        if(!is_numeric($receiver_address_id))
    ...
    public function edit()
    {
        ...
        if ($this->check->check_ajax_form($rule, false) === false) {
          $this->json->set_error('002', validation_errors());
        }
        $f_data = $this->form_data($rule);
    ...
    {
        $uid = $this->s_user->uid;;
        $receiver_address_id = $this->input->post('receiver_address_id');
        $is_default = $this->input->post('is_default');
        if(!is_numeric($receiver_address_id) || !is_numeric($is_default))
            $this->json_do->set_error('001', '参数错误');

        $wmReceiverAddressDao = WmReceiverAddressDao::i($this->aid);
        $wmReceiverAddressDao->update(['is_default'=>0], ['uid'=>$uid, 'aid'=>$this->aid]);
        $wmReceiverAddressDao->update(['is_default'=>1], ['id'=>$receiver_address_id,'uid'=>$uid, 'aid'=>$this->aid]);
