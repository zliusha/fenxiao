...
class Payment extends meal_table_controller
{
    // 获取结算金额
    public function checkout()
    {
        ...
        if ($mealOrderDao->getCount(['status' => 1010, 'order_table_id' => $fdata['order_table_id'], 'aid' => $this->aid]) > 0) {
            $this->json_do->set_error('004', '当前存在待审核订单，请跟服务员确认后再结算');
        }

        // 查询待支付金额总和
        $sum_pay_money = $mealOrderDao->getSum('pay_money', ['status' => 2020, 'order_table_id' => $fdata['order_table_id'], 'aid' => $this->aid]);
    ...
    public function weixin()
    {
        ...
        // 查询桌位订单记录表
        $mealOrderTableDao = MealOrderTableDao::i($this->aid);
        $order_table = $mealOrderTableDao->getOne(['id' => $fdata['order_table_id'], 'aid' => $this->aid]);
        ...
        // 查询待支付金额总和
        $mealOrderDao = MealOrderDao::i($this->aid);
        $sum_pay_money = $mealOrderDao->getSum('pay_money', ['status' => 2020, 'order_table_id' => $order_table->id, 'aid' => $this->aid]);
        ...
        if (in_array(ENVIRONMENT, ['development'])) {
            ...
            $result = $mealPaymentRecordDao->weixinOrderCallback($data['code'], $out_trade_no);

            if ($result) {

                //获取支付流水信息
                $mealStatementsDao = MealStatementsDao::i($this->aid);
                $m_meal_statements = $mealStatementsDao->getOne(['order_number' => $out_trade_no, 'type' => 1, 'aid' => $this->aid], 'id,order_table_id,aid,shop_id,type,order_number,amount,gateway,time');
    ...
    public function weixin_result()
    {
        ...
        $mealPaymentRecordDao = MealPaymentRecordDao::i($this->aid);
        $result = $mealPaymentRecordDao->getOne(['code' => $fdata['code'], 'gateway' => 'weixin', 'aid' => $this->aid], 'status', 'id desc');
