...
class Xitems extends xmeal_controller
{
    // 所有分类商品列表
    public function cate_goods_list()
    {
        ...
        $goods_config_arr = array(
            ...
            'where' => " a.aid={$this->aid} AND a.shop_id={$shop_id} AND a.is_delete=0 AND a.status=1",
        );
    ...
    public function recommend_goods_list()
    {
        ...
        $goods_config_arr = array(
            'field' => $fields,
            'table' => "{$wmShardDb->tables['wm_goods']} a",
            'where' => " a.aid={$this->aid} AND a.shop_id={$shop_id} AND a.is_delete=0 AND a.status=1 AND a.tag=1",
            'limit' => 20
        );
