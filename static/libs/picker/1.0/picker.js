...
(function (window, document, Math) {
  ...
  function Scroll(id, params) {
    ...
    // 默认列表位置
    var defaultPlace = this.options.defaultPlace ?
    ...
    _end: function () {
      ...
      self.scroller.addEventListener(
        ...
        function (e) {
          ...
          if (
            self.offsetTop > 0 ||
            Math.abs(self.offsetTop) > Math.abs(self.scrollHeight)
          ) {
            //上边缘&下边缘
          ...
          if (self.options.step && self.stepLen > 0) {
            ...
            //超过行一半的高度，则滚动一行
            var moveY =
              h >= halfHeight ? nowEndY - self.stepLen + h : nowEndY + h;
  ...
  Picker.prototype = {
    ...
    FillData: function () {
      ...
      var picker_list =
        '<div class="zx_select showPicker">' +
        '<header><button class="nav_left picker-cancel">取消</button>' +
        "<h1>" +
        title +
        '</h1><button class="nav_right picker-ok">确定</button></header>' +
        '<div class="ub"><div class="ub-f1 picker-wrapper" id="picker-wrapper0"><ul></ul></div>';

      //2级选择
      if (self.arrayDepth >= 2) {
        picker_list +=
          '<div class="ub-f1 picker-wrapper" id="picker-wrapper1"><ul></ul></div>';
      }

      //3级选择
      if (self.arrayDepth >= 3) {
        picker_list +=
          '<div class="ub-f1 picker-wrapper" id="picker-wrapper2"><ul></ul></div>';
      }

      picker_list +=
        '<div class="sel_top"></div><div class="sel_bottom"></div>';
      ...
      // setTimeout(function () {
      //   document.querySelector(".zx_select").style.height = "12rem";
      // }, 0);
    },
    ...
    scrollInit: function (index, num) {
      ...
      //每个选项对的高度
      var wrapperList = document.querySelector("#picker-wrapper0")
        .childNodes[0];
    ...
    showScrollList: function (dataList, index, isInit) {
      ...
      if (self.defaultArray[index]) {
        ...
        for (var i = 0; i < dataList.length; i++) {
          if (
            self.defaultArray[index] &&
            self.defaultArray[index] == dataList[i][keyValue]
          ) {
            isMatch = true;
            defaultNum = i; //默认选中的值
