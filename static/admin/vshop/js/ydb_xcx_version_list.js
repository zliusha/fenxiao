...
$(function () {
  ...
  function getYdbList(curr) {
    ...
    $.getJSON(__BASEURL__ + 'wm_main_xcx_version_api/grid_data', {
      title:title,
      current_page: curr || 1,
      page_size: page_size
    }, function (data) {
  ...
  $delConfirm.on('click', function () {
    ...
    $.post(__BASEURL__ + 'wm_main_xcx_version_api/ids_del', autoCsrf({
      ids: id
    }), function (data) {
      if (data.success) {
        new Msg({
  ...
  function releaseNotice(id, status) {
    $.post(
      __BASEURL__ + "wm_main_xcx_version_api/status_update",
      autoCsrf({
        id:id,
        status: status
      }),
